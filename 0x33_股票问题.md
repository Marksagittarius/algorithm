...
public:
    int maxProfit(int k, vector<int>& prices) {
        int n = prices.size();
        vector<vector<int> > f(n, vector<int>(2 * k + 1));
        for (int i = 0; i < k; i++) f[0][2 * i + 1] = -prices[0];
        for (int i = 1; i < n; i++) {
            f[i][0] = f[i - 1][0];
            for (int j = 1; j <= k; j++) {
                f[i][2 * j - 1] = max(f[i - 1][2 * j - 1], f[i - 1][2 * j - 2] - prices[i]);
                f[i][2 * j] = max(f[i - 1][2 * j], f[i - 1][2 * j - 1] + prices[i]);
            }
        }
        return f[n - 1][2 * k];
    }
