...
public:
    int f(vector<int>& nums, int l, int r, int k) {
        ...
        if (k <= j) return f(nums, l, j, k);
        return f(nums, j + 1, r, k);
    }
    int findKthLargest(vector<int>& nums, int k) {
        int n = nums.size();
        return f(nums, 0, n - 1, n - k);
    }
};
```
...
堆排序需要借助优先队列（堆）这一数据结构进行排序，其具体应用将在第二章进行详细介绍。
...
该问题是一个综合性的算法优化问题，融会贯通了排序、二分查找技巧与后续章节中会介绍的最长递增子序列问题。
